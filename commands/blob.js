...
exports.run = (client,message, args) => {
    ...
    if (args[0] == "cm"){
        ...
        SendMessagetoGroup(client,'\:warning: Le **'+message.channel.name+'** utilise **1 Contre-Mesure** , il en reste **'+contreMesure+'**') 
    }

    if (args[0] == "i"){
        if (!isNaN(args[1])){
            indice  = indice - parseInt(args[1])
            SendMessagetoGroup(client,'Le **'+message.channel.name+'** depose **'+parseInt(args[1])+' Indice(s)** , il en reste **'+indice+'**') 
        }else
        {
            indice = indice--
            SendMessagetoGroup(client,'Le **'+message.channel.name+'** depose **1 Indice** , il en reste **'+indice+'**') 
        }       
    }
    if (args[0] == "reset"){
        if (!isNaN(args[1])){
            indice  = initialIndice
            SendMessagetoGroup(client,'Reinitialisation du nombre d indice Acte 1 à **'+indice+'**') 
        }  
    }    
}

function SendMessagetoGroup(myclient,text) {
    myclient.channels.cache.filter(chan => chan.name.startsWith("group").forEach(channel => { channel.send(text)})) 
}
